                ...
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                        <div class="flex items-center gap-3 mb-2">
                            ...
                            @if($hasFirstAttempt)
                                <span class="text-xs px-2 py-1 rounded-md border border-yellow-500 bg-yellow-900 text-yellow-200 font-semibold">
                                    Unang Iskor: {{ $firstAttempt->score }} / 50
                                </span>
                            @endif
                        </div>
                        <p class="text-sm md:text-base text-yellow-100/80">
                        ...
                        @if($hasFirstAttempt)
                            <p class="text-xs text-yellow-300/60 mt-2">
            ...
            @foreach($sections as $section)
                <div class="flex flex-col gap-4">
                    ...
                    <div class="rounded-xl border border-white/30 bg-white/5 px-6 py-5">
                        <p class="text-xs uppercase tracking-widest mb-3 !text-[#F4C300]">{{ $section['label'] }}</p>

                        @if($section['title'])
                            <h2 class="text-lg font-bold text-[#F4C300] mb-1">{{ $section['title'] }}</h2>
                        @endif
                        ...
                        @if($section['type'] === 'tula')
                            <div class="border-l-2 border-[#F4C300]/40 pl-4 flex flex-col gap-1 text-sm md:text-base italic text-white/80">
                    ...
                    @foreach($section['range'] as $num)
                        ...
                        <div class="rounded-xl border border-white/10 bg-white/5 px-6 py-5 flex flex-col gap-4
                            ...
                            <p class="text-sm md:text-base font-medium text-white leading-relaxed">
                                <span class="text-[#F4C300] font-bold mr-2">{{ $num }}.</span>{{ $item['q'] }}
                            </p>
                                ...
                                @foreach($item['choices'] as $index => $choice)
                                    ...
                                    <label class="flex items-start gap-3 {{ $hasFirstAttempt ? 'cursor-not-allowed' : 'cursor-pointer group' }}">
                                        <input
                                            ...
                                            name="question_{{ $num }}"
                                            value="{{ $letter }}"
                                            class="mt-0.5 accent-[#F4C300] w-4 h-4 shrink-0 {{ $hasFirstAttempt ? 'cursor-not-allowed' : 'cursor-pointer' }}"
            ...
            <div class="flex justify-end pb-4">
                @if($hasFirstAttempt)
                    <button
                        ...
                        class="px-8 py-3 border border-[#F4C300] text-[#F4C300] rounded-md font-bold text-sm md:text-base hover:bg-[#F4C300]/10 transition-colors"
                    >
                        Subukan Muli
                    </button>
                @else
                    <button
                        type="submit"
                        class="px-8 py-3 bg-[#F4C300] !text-black rounded-md font-bold text-sm md:text-base hover:bg-yellow-300 transition-colors"
                    >
                        Isumite ang Sagot
                    </button>
                @endif
