...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body>
            ...
            <template x-for="n in notifs" :key="n.id">
                ...
                >
                    <div class="notif-header font-bold text-lg flex items-center relative">
                        ...
                        <i class="fa-solid fa-xmark absolute -right-8 text-gray-500 hover:text-gray-300 text-xl cursor-pointer" @click="n.show = false"></i>
                    </div>
        ...
        <nav>
            <div class="logo">
                <img class="img-logo" src="{{ asset('img/logo-light.png') }}" style="width: 130px;" alt="Logo">
        ...
        {{-- ===================== MAIN CONTENT ===================== --}}
        {{-- flex-1 + min-w-0: fills remaining body width without overflowing --}}
        {{-- overflow-hidden: completes flex chain so inner containers scroll internally --}}
        <main class="flex-1 min-w-0 overflow-hidden flex flex-col gap-5 pb-2">

            <header class="flex justify-between items-center flex-shrink-0">
                <div>
                    <div class="text-sm text-gray-400">Pages /
                        <span>
                            @if(request()->is('teacher/dashboard*')) Dashboard
                            @elseif(request()->is('teacher/studentmanagement*')) Student Management
                            @elseif(request()->is('teacher/teachermanagement*')) Teacher Management
                            @elseif(request()->is('teacher/performancereport*')) Performance Report
                            @elseif(request()->is('teacher/settings*')) Settings
                            @endif
                        </span>
                    </div>
                ...
                <div class="text-sm text-gray-400">
                    Hi, <span class="font-semibold text-white">
                        @auth('teacher') {{ auth('teacher')->user()->name }} @endauth
                    </span>
                </div>
            </header>

            {{-- flex-1 + min-h-0: lets yielded content participate in height chain --}}
